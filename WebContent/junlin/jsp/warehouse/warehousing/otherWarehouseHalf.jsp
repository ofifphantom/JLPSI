...
<head>
		...
		<div class="page-content clearfix">
			<div id="Member_Ratings">
				<div class="d_Confirm_Order_style" style="margin-top:10px;">
					<h4 class="text-title">其他收货单部分入库单</h4>
					<div class="search-div clearfix">
						...
						<span class="l_f"> 
							状态：<select id="query_state" name="">
								...
								<option value="3" >部分入库终止</option>
								<option value="11" >已送至采购开单</option>
					...
					"url": "<%=basePath%>purchase/procuretable/getProcureTableMsg"
							...
							"aoColumns" : [
...

									{
										...
										"sClass" : "center",
										"mRender" : function(data, type, row) {
											if(row.state==11||row.state==10||row.state==9){
												return '<td><input type="button" class="btncss edit" onclick="WarehouseReceiptDetail('+data+')" value="详情" />'
												+'<input type="button" class="btncss edit" onclick="WarehouseHalfApplyBilling('+row.parentId+')" value="通知开单" /></td>'
											}
											else{
	...
	function WarehouseReceiptDetail(id) {
		...
		<%-- $.ajax({
			...
			success : function(data) {
				...
				if(data.isOtherReceipts!=1){
					$("#purchaseOrderDiv1").removeClass("hidden");
					$("#purchaseOrderDiv2").removeClass("hidden");
					if(data.supcto == null){
						$("#look_supctoId").html("");
					}else{
					...
					$("#purchaseOrderDiv1").addClass("hidden");
					$("#purchaseOrderDiv2").addClass("hidden");
				...
				for(var i=0;i<data.procureCommoditys.length;i++){
					appendGoodsDiv(i+1);
					if(data.state==15||data.state==13){
						$(".this_arrivalQuantity").addClass("hidden");
					}
					else{
						$(".this_arrivalQuantity").removeClass("hidden");
					}
					...
					$("#goodsMsg"+(i+1)+" .look_procureCommodity_all_arrivalQuantity").html(data.procureCommoditys[i].orderNum-data.procureCommoditys[i].suspendQuantity);
		...
		$.ajax({
			type: "post",
			url: "<%=basePath%>purchase/procuretable/partInputOrderDetail",
			dataType : "json",
			data: {
				"id" : id
			},
	...
	function appendGoodsDiv(index){	
		...
		<div class="form-group">
			<label for="" class="col-xs-4 control-label">共到货数量：</label>
			<div class="col-xs-8 look_procureCommodity_all_arrivalQuantity">123456</div>
		</div>
	...
	<div class="col-xs-6">
		...
		<div class="form-group this_arrivalQuantity" >
			<label for="" class="col-xs-4 control-label">本次到货数量：</label>
	...
	/* 部分入库送至采购开单 */
	function WarehouseHalfApplyBilling(parentId){
		publicMessageLayer("通知采购开单", function() {
			 ajaxJquery("POST","<%=basePath%>purchase/procuretable/warehouseHalfApplyBilling",{"parentId":parentId},function(data){
				if(data.success) {
					laysuccess(data.msg);
