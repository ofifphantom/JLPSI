		...
		<div class="page-content clearfix">
			<div id="Member_Ratings">
				<div class="d_Confirm_Order_style" style="margin-top:10px;">
					<h4 class="text-title">其他收货单引用</h4>
	...
	jQuery(function($) {
							...
							"aoColumns" : [
...

									{
										...
										"mRender" : function(data, type, row) {
											...
											+'<input type="button" class="btncss edit" onclick="warehousingAll('+data+')" value="全部入库" />'
											+'<input type="button" class="btncss edit" onclick=\'warehousing(' + JSON.stringify(row) + ')\' value="部分入库" /></td>';
		...
		function appendallGoods(goodsarr){
			for(let i=0;i<goodsarr.length;i++){
				...
				$tr = $tr + getJqueryHtmlCode($select) + `</td>
				<td id="arrivalQuantity">` + (goodsarr[i]["orderNum"]- goodsarr[i]["arrivalQuantity"])+`(` +(goodsarr[i]["orderNum"]) + `)</td>
		...
		function procureTableDataToJSON(allOrPart){
			...
			//采购订单基础的信息
			 var  procureTableDataJSON={"id":$("#pOrderId").html(),"state":$("#pState").html(),"isOtherReceipts":1,"procureCommoditys":[]}; 
			if(allOrPart==1){
				//获取采购商品的信息，添加到采购订单的商品信息里
