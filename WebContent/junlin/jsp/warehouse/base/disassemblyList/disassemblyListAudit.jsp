...
<head>
				...
				<div class="d_Confirm_Order_style" style="margin-top:10px;">
					...
					<div class="search-div clearfix">
						<div class="clearfix">
							...
							<span class="l_f"> 
								制单人姓名： <input type="text" id="search_originator"  value="" onblur="cky(this)" />
							</span>
							<span class="r_f"> 
						...
						<table class="table table-striped table-hover col-xs-12" id="datatable">
							...
							<tbody>
								<!-- <tr>
									...
									<td>2017.10.02</td>
									<td>
										<input type="button" class="btncss edit" onclick="approval()" value="通过" />
										<input type="button" class="btncss edit" onclick="dismissal()" value="驳回" />
									</td>
								</tr> -->
							</tbody>
						</table>
					...
					"url": "<%=basePath%>warehouse/packageOrTeardownOrder/getDataTablesMsg"
							...
							"aoColumns" : [
...

									{
										...
										"mRender" : function(data, type, row) {
											if(row.state==2){
												return '<td><input type="button" class="btncss edit" onclick=\'approval('+JSON.stringify(row)+')\' value="通过" />'
												+'<input type="button" class="btncss edit" onclick=\'dismissal('+JSON.stringify(row)+')\' value="驳回" /></td>'
											}
	...
	$(function(){
		...
		function approval(row) {
			publicMessageLayer("通过", function() {
				ajaxJquery("GET","<%=basePath%>warehouse/packageOrTeardownOrder/updateOrderState",{"orderId":row.id,"orderIdentifier":row.identifier,"state":3,"orderType":2},function(data){
					if(data.success) {
						laysuccess(data.msg);
		...
		function dismissal(row) {
			publicMessageLayer("驳回", function() {
				ajaxJquery("GET","<%=basePath%>warehouse/packageOrTeardownOrder/updateOrderState",{"orderId":row.id,"orderIdentifier":row.identifier,"state":4,"orderType":2},function(data){
					if(data.success) {
						laysuccess(data.msg);
						oTable1.fnDraw(false);
						$("#checkAll").removeAttr("checked");
