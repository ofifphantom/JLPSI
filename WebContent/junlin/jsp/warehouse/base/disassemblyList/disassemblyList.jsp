...
<head>
			...
			<div class="print-content">
				...
				<form class="container">
					...
					<div class="row">
						...
						<div class="col-xs-4">
							<div class="form-group">
								<label for="" class="col-xs-5 control-label">拆卸数量：</label>
					...
					<input type="text" class="hidden" value="" id="packageOrTeardownOrderId" />
						...
						<div class="col-xs-6">
							<div class="form-group">
								...
								<div class="col-xs-8">
									<select class="form-control" id="ware_house_id" disabled></select>
								</div>
							</div>
						</div>
						<div class="col-xs-6">
							<div class="form-group">
								<label for="" class="col-xs-4 control-label">拆卸数量</label>
								<div class="col-xs-8">
						...
						<div class="col-xs-6">
							<div class="form-group">
								...
								<div class="col-xs-8">
									<input type="text" id="amountMoney" class="form-control" placeholder="请先填写拆卸数量" disabled="disabled"/>
								</div>
							</div>
						</div>
...

									{
										...
										"mRender" : function(data, type, row) {
											if(row.isDelete==0){
												...
												return '<td><input type="button" class="btncss edit" onclick="assemblySheetDetail(' + data + ')" value="详情" /></td>'
	...
	$(function(){
		...
		ajaxJquery("GET","<%=basePath%>basic/warehouse/selectAllWarehouse",{},function(data){
			warehouseData=data;
			addWarehouseOption($("#search_warehouse"),warehouseData);
			addWarehouseOption($("#ware_house_id"),warehouseData);	
		})
	})
	
	/*仓库添加option*/
	function addWarehouseOption($select,warehouseData){
			$select.html("<select class='form-control'></select>");
			if(warehouseData.length==0){
                $select.append("<option value='-1' selected>--暂无数据，请到仓库页面进行添加。--</option>");
            }
            else{
                $select.append("<option value='-1' selected>--请选择仓库--</option>");
                for(var i=0;i<warehouseData.length;i++){
                    var option = $("<option value='"+warehouseData[i].id+"'>" + warehouseData[i].name+"</option>");
	...
	function selectCommodityMsg(e,selectValId){	
			...
			layer.open({
				...
				yes: function(index) {
					...
					packageOrTeardownOrderCommodityMsg();
		 			if(commodityTotalMoney!=$("#amountMoney").val()){
						MessageLayer("拆卸商品总价与拆卸后不一致，确定继续？", function() {
							ajaxJquery("GET","<%=basePath%>warehouse/packageOrTeardownOrder/decideCommodtyInventory",{"ids":ids,"commodityNums":commodityNums,"orderType":2},function(data){
								if(data.success){
									ajaxJquery("GET","<%=basePath%>warehouse/packageOrTeardownOrder/createOrder",{"orderType":2,"isHasWarehouse":true,"PackageOrTeardownOrder":JSON.stringify(packageOrTeardownOrderDataToJSON())},function(data){
										if(data.success) {
											laysuccess(data.msg);
		...
		function assemblySheetModify(id) {
				...
				if(data!=null){
					...
					if(data.packageOrTeardownOrderCommodities.length>0){
						for(var i=0;i<data.packageOrTeardownOrderCommodities.length;i++){
							...
							$(".tipTr").remove();
							$("#goodsTbody").append('<tr class="package_or_teardown_order_commodity_tr"><td class="poocId hidden">'+data.packageOrTeardownOrderCommodities[i].id+'</td><td class="goodsId hidden">' + data.packageOrTeardownOrderCommodities[i].commoditySpecificationId + '</td>'+
								...
								'<td><select class="form-control ware_house" attr-name="仓库"></select></td>'+
								'<td><input type="text" class="form-control number" onkeyup="countGoodsMoney(this)" attr-name="数量" value="'+data.packageOrTeardownOrderCommodities[i].number+'" maxlength="9"/></td><td class="goodsUnitPrice">' + data.packageOrTeardownOrderCommodities[i].unitPrice + '</td>' +
							...
							addWarehouseOption($("#goodsTbody tr .ware_house:last"),warehouseData);	
							if(data.packageOrTeardownOrderCommodities[i].specWarehouseId!=null&&data.packageOrTeardownOrderCommodities[i].specWarehouseId>0){
								$("#goodsTbody tr .ware_house:last").val(data.packageOrTeardownOrderCommodities[i].specWarehouseId);
								$("#goodsTbody tr .ware_house:last").attr("disabled","disabled");
							}
							else{
								$("#goodsTbody tr .ware_house:last").val("-1");
								$("#goodsTbody tr .ware_house:last").removeAttr("disabled");
							}
						}
					}
				}
			...
			layer.open({
				...
				yes: function(index) {
					...
					ajaxJquery("GET","<%=basePath%>warehouse/packageOrTeardownOrder/decideCommodtyInventory",{"ids":ids,"commodityNums":commodityNums,"orderType":2},function(data){
						if(data.success){
							ajaxJquery("GET","<%=basePath%>warehouse/packageOrTeardownOrder/updateOrder",{"orderType":2,"PackageOrTeardownOrder":JSON.stringify(packageOrTeardownOrderDataToJSON())},function(data){
								if(data.success) {
									laysuccess(data.msg);
									oTable1.fnDraw(false);
									$("#checkAll").removeAttr("checked");
		...
		function assemblySheetDetail(id) {
			...
			ajaxJquery("GET","<%=basePath%>warehouse/packageOrTeardownOrder/getPackageOrTeardownOrderDetailById",{"id":id},function(data){
				if(data!=null){
					...
					$("#allgoodDivDetail table tbody").html("<tr><th>货品编码</th><th>货品名称</th><th>规格</th><th>品牌</th><th>条形码</th><th>单位</th><th>仓库</th><th>数量</th><th>单价</th><th>金额</th><th>产品批号</th><th>生产日期</th><th>有效期至</th></tr>");
					if(data.packageOrTeardownOrderCommodities.length>0){
					...
					$("#allgoodDivDetail table tbody").append('<tr><td>合计</td><td></td><td></td><td></td><td></td><td></td><td></td><td>'+total_num+'</td><td></td><td>'+total_money+'</td><td></td><td></td><td></td></tr>');
				}
			}) --%>
			 $("#detailDiv").html("");
				$.ajax({
		...
		function assemblySheetDel(row) {
			publicMessageLayer("删除所选单据", function() {
				ajaxJquery("GET","<%=basePath%>warehouse/packageOrTeardownOrder/deleteOrderById",{"orderId":row.id,"orderIdentifier":row.identifier,"orderType":2},function(data){
					if(data.success) {
						laysuccess(data.msg);
						oTable1.fnDraw(false);
						$("#checkAll").removeAttr("checked");
		...
		function sendTo(row) {
			publicMessageLayer("送审", function() {
				ajaxJquery("GET","<%=basePath%>warehouse/packageOrTeardownOrder/updateOrderState",{"orderId":row.id,"orderIdentifier":row.identifier,"state":2,"orderType":2},function(data){
					if(data.success) {
						laysuccess(data.msg);
						oTable1.fnDraw(false);
						$("#checkAll").removeAttr("checked");
		...
		function goodsAddEdit() {
			...
			if(goodsId == -1) {
				...
				$("#goodsTbody").append('<tr class="package_or_teardown_order_commodity_tr"><td class="poocId hidden"></td><td class="goodsId hidden">' + goodsId + '</td>'+
					'<td class="isHasWarehouse hidden">'+isHasWarehouse+'</td><td class="identifier">' + identifier + '</td><td>' + name + '</td><td>' + type + '</td><td>' + unit + '</td>' +
					'<td><select class="form-control ware_house" attr-name="仓库"></select></td>'+
				...
				addWarehouseOption($("#goodsTbody tr .ware_house:last"),warehouseData);	
				if(isHasWarehouse){
					$("#goodsTbody tr .ware_house:last").val(warehouseId);
					$("#goodsTbody tr .ware_house:last").attr("disabled","disabled");
				}
				else{
					$("#goodsTbody tr .ware_house:last").val("-1");
		...
		/*清空商品*/
		function clearGoods() {
			$("#goodsTbody").html('<tr><th>货品编码</th><th>货品名称</th><th>规格</th><th>单位</th><th>仓库</th><th>数量</th><th>单价</th><th>金额</th><th>操作</th></tr>' +
		...
		function countMoney(thisInput) {
			...
			if(isnotEmpty($(thisInput).val())){
				...
				var amountMoney=($("#unitPrice").val() - 0) * ($(thisInput).val() - 0);
				$("#amountMoney").val(toDecimal2(amountMoney));
			}else{
				$("#amountMoney").val("");
			}
		...
		function packageOrTeardownOrderDataToJSON(){
			...
			$(".package_or_teardown_order_commodity_tr").each(function(index, val){
				...
				var packageOrTeardownOrderCommodityDataJSON={"id":$(val).find(".poocId").html(),"packageOrTeardownOrderId":$("#packageOrTeardownOrderId").val(),"commoditySpecificationId":$(val).find(".goodsId").html(),
						"number":$(val).find(".number").val(),"unitPrice":$(val).find(".goodsUnitPrice").html(),"money":$(val).find(".goodsAllPrice").val(),"specWarehouseId":$(val).find(".ware_house").val(),"poocIsHasWarehouse":$(val).find(".isHasWarehouse").html()};	
		...
		let ids=[];
		let commodityNums=[];
		let commodityTotalMoney=0;//获取拆卸单商品的总金额
		/* 获取拆卸单商品的id以及需要拆卸的个数，传入后台判断库存是否满足 */
		function packageOrTeardownOrderCommodityMsg(){
			ids=[];
			commodityNums=[];
			ids.push($("#edit_giftBox").val());
			commodityNums.push($("#package_or_teardown_num").val());
			console.log("ids",ids);
			console.log("commodityNums",commodityNums);
			commodityTotalMoney=0;
			$(".package_or_teardown_order_commodity_tr").each(function(index, val){
				commodityTotalMoney+=$(val).find(".goodsAllPrice").val()-0;
			});
