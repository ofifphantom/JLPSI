...
<html>
	...
	<body class="content">
		...
		<div id="editDiv" style="display: none;">
			<form class="container">
			<input type="text" class="form-control hidden" id="edit_breakageOrderId" />
			<input type="text" class="form-control hidden" id="edit_breakageOrderIdentifier" />
	...
	jQuery(function($) {
							...
							"aoColumns" : [{
									...
									},
									{
										...
										"mRender" : function(data, type, row) {
											...
											if(row.isDelete==0){
												if(row.state == 1){
													return '<td><input type="button" class="btncss edit" onclick="breakageOrderModify('
													+ data
													+')" value="编辑" />'
													+ '<input type="button" class="btncss edit" onclick="breakageOrderDel('
													+ data
													+ ')" value="删除" />'
													...
													return '<td><input type="button" class="btncss edit" onclick="breakageOrderDetail(' + data + ')" value="详情" /></td>'
												}
											}else{
												return '<td><input type="button" class="btncss edit" onclick="breakageOrderDetail(' + data + ')" value="详情" /></td>'
											}
		...
		function reportLossAdd() {
			...
			layer.open({
				...
				yes: function(index) {
					...
					$.ajax({
						url: '<%=basePath%>warehouse/base/breakageOrder/addBreakageOrder',
						type: "POST",
						dataType: "json",
						data:{
							"breakageOrder":JSON.stringify(breakageOrderDataJSON())
							},
						async: false,
						cache: false,
						success: function(data) {
		...
		/*编辑*/
		function breakageOrderModify(id) {
			ajaxJquery("GET","<%=basePath%>warehouse/base/breakageOrder/selectBreakageOrderDetailById",{"id":id},function(data){
				$("#edit_breakageOrderId").val(id);
				$("#edit_breakageOrderIdentifier").val(data.identifier);
				...
				for (var i = 0; i < data.breakageOrderCommodities.length; i++) {
					...
					let inventory = commoditie.commoditySpecification.inventories[0].inventory;
					...
					$("#goodsTbody").append('<tr class="goodsTr"><td class="commoditySpecificationId" style="display:none">'+goodsId+'</td><td class="specificationidentifier">' + 
							...
							baseunitname + '</td><td class="inventory">' +
							inventory + '</td><td><input type="text" class="number form-control" value="'+number+'" onkeyup="countGoodsMoney(this)" maxlength="9" attr-name="报损数量"/></td><td class="movingaverageprice">' +
							movingaverageprice + '</td><td><input type="text" class="goodsAllPrice form-control" value="'+money+'" placeholder="请先填写数量" disabled="disabled"/></td><td>' +'<input type="button" class="btncss edit" onclick="goodsDel(this)" value="删除" /></td></tr>');
			...
			layer.open({
				...
				yes: function(index) {
					...
					$.ajax({
						url: '<%=basePath%>warehouse/base/breakageOrder/editBreakageOrder',
						type: "POST",
						dataType: "json",
						data:{
							"breakageOrder":JSON.stringify(breakageOrderDataJSON())
		...
		function breakageOrderDetail(id) {
			<%-- $.ajax({
				...
				success: function(data) {
					...
					$("#look_goods").append('<tr><td>合计</td><td></td><td></td><td></td><td></td><td></td<td></td<td></td><td>'+total_num+'</td><td></td><td>'+total_money+'</td><td></td><td></td><td></td></tr>');
		...
		/*删除*/
		function breakageOrderDel(id){
			publicMessageLayer("删除该单据", function() {
				$.ajax({
					url :'<%=basePath%>warehouse/base/breakageOrder/deleteBreakageOrder',
					type: "POST",
					dataType: "json",
					async: false,
					cache: false,
		...
		/*发送到财务*/
		function sendTo(id) {
			publicMessageLayer("将该单据发送至财务", function() {
				let ids = [id];
				$.ajax({
		...
		function goodsAddEdit() {
			...
			if($("#edit_warehouseId").val()=="-1"){
				...
				let inventory = $("#edit_goods option:selected").attr("data-inventoryNum");
				...
				$("#goodsTbody").append('<tr class="goodsTr"><td class="commoditySpecificationId" style="display:none">'+goodsId+'</td><td class="specificationidentifier">' + 
						...
						specificationname + '</td><td class="baseunitname">' +
						baseunitname + '</td><td class="inventory">' +
						inventory + '</td><td><input type="text" class="number form-control" onkeyup="countGoodsMoney(this)" maxlength="9" attr-name="报损数量"/></td><td class="movingaverageprice">' +
						movingaverageprice + '</td><td><input type="text" class="goodsAllPrice form-control" placeholder="请先填写数量" disabled="disabled"/></td><td>' +'<input type="button" class="btncss edit" onclick="goodsDel(this)" value="删除" /></td></tr>');
		...
		/*清空商品*/
		function clearGoods() {
			$("#goodsTbody").html('<tr><th>货品编码</th><th>货品名称</th><th>规格</th><th>单位</th><th>现有库存</th><th>报损数量</th><th>单价</th><th>金额</th><th>操作</th></tr>' +
		...
		function countGoodsMoney(thisInput) {
			if(isnotEmpty($(thisInput).val())){
				...
				let unitPrice = $(thisInput).parents("tr").find(".movingaverageprice").html();
				let inventory = $(thisInput).parents("tr").find(".inventory").html();
				if((count-0)>(inventory-0)){
					laywarn("报损数量不能大于现有库存");
					$(thisInput).val("");
		...
		function orderPrint(row) {
			...
			ajaxJquery("GET","<%=basePath%>warehouse/base/breakageOrder/printBreakageOrder",{"id":row.id},function(data){
			...
			layer.open({
				...
				yes: function(index) {
					$.ajax({
						url :'<%=basePath%>warehouse/base/breakageOrder/updateBreakageOrderPrintNum' ,
						type : "POST",
						dataType : "json",
						data: {
							"id" : row.id
    	...
    	function breakageOrderDataJSON(){
    		//盘点单基础的信息
    		 var  breakageOrderDataJSON={
    				"id":$("#edit_breakageOrderId").val(),
    				"identifier":$("#edit_breakageOrderIdentifier").val(),
    				"warehouseId":$("#edit_warehouseId").val(),
    		...
    		$("#goodsTbody .goodsTr").each(function(index, val){
    			var breakageOrderCommoditiesDataJSON={
    					"commoditySpecificationId":$(val).find(".commoditySpecificationId").html(),
    					"number":$(val).find(".number").val(),
    					"unitPrice":$(val).find(".movingaverageprice").html(),
    					"money":$(val).find(".goodsAllPrice").val()
    				};	
    				
    			breakageOrderDataJSON.breakageOrderCommodities[index]=breakageOrderCommoditiesDataJSON;	
    		});
    			
    		
    		return breakageOrderDataJSON; 
    	}
	</script>
