	...
	$(function(){
		...
		function approval(row) {
			publicMessageLayer("通过", function() {
				ajaxJquery("GET","<%=basePath%>warehouse/packageOrTeardownOrder/updateOrderState",{"orderId":row.id,"orderIdentifier":row.identifier,"state":3,"orderType":1},function(data){
					if(data.success) {
						laysuccess(data.msg);
		...
		function dismissal(row) {
			publicMessageLayer("驳回", function() {
				ajaxJquery("GET","<%=basePath%>warehouse/packageOrTeardownOrder/updateOrderState",{"orderId":row.id,"orderIdentifier":row.identifier,"state":4,"orderType":1},function(data){
					if(data.success) {
						laysuccess(data.msg);
						oTable1.fnDraw(false);
						$("#checkAll").removeAttr("checked");
