...
<head>
	...
	jQuery(function($) {
					...
					"url": "<%=basePath%>sales/salesOrder/getSalesOrderMsg"
							...
							"aoColumns" : [
...

									{
										...
										"sClass" : "center",
										"mRender" : function(data, type, row) {
											if(row.state==35||row.state==9){
												return '<td><input type="button" class="btncss edit" onclick="salesOrderDetail('+data+')" value="详情" />'
												+'<input type="button" class="btncss edit" onclick=\'goToStockUp('+JSON.stringify(row)+')\' value="去备货" />'
												+'<input type="button" class="btncss edit" onclick=\'printOutLibraryList('+JSON.stringify(row)+')\' value="打印备货单" /></td>'
		...
		/* 去备货 */
		function goToStockUp(row){
			publicMessageLayer("去送去备货", function() {
				ajaxJquery("GET","<%=basePath%>sales/salesOrder/goToStockUp",{"id":row.id,"identifier":row.identifier},function(data){
					if(data.success) {
						laysuccess(data.msg);
		...
		function printOutLibraryList(row) {
			...
			let json={"id":row.id};
			ajaxJquery("GET","<%=basePath%>sales/salesOrder/printOutboundOrder",{"id":row.id,"isOutBoundOrPickingList":0},function(data){
				console.log("打印");
				console.log(data);
					let bill=new Bill(data);
					let $content=bill.toPrint();
					$("#pickingList_div").html("");
					$("#pickingList_div").append($content);
					$("#pickingList_div  .container").append(watermark("打印次数："+(row.printNum+1)+"次","#pickingList_div"));
				...
				yes:function(index){
					$.ajax({
						...
						success : function(data) {
							$("#pickingList_div .container").printArea();
							layer.close(index);
							oTable1.fnDraw(false);
						}
					});
