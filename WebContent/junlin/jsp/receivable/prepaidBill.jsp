...
<html lang="en">
	...
	laydate.render({
		...
		function advancesReceivedAdd() {
			...
			orderBillAdd("预付款单",function(index){
				/*
				 * 请在此处添加点击提交按钮后的代码
				 */
	                ...
	                success: function(data) {
	        			...
	        			//循环加载新数据
	               		...
	               		for(var i=0;i<subList.length;i++){
	               			...
	               			var tr="<tr><td>"+subList[i].identifier+"</td><td>"+orderTime+"</td><td>"+subList[i].orderMoney+"</td><td>"+clearingMoney+"</td><td>"+subList[i].theMoeny+"</td>"+
	               			"<td>"+subList[i].remark+"</td><td>采购订单</td></tr>";
		...
		function documentReference() {
			...
			let clearingMoney=$("#bill option:selected").attr("attr-alreadyPrice");
		...
		/* 		var billsSubsDataJSON={"id":$(val).find(".subId").val(),"billsId":$("#edit_id").val(),"procureSalesId":$(val).find(".procureSalesId").val(),"clearingMoney":$(val).find(".clearingMoney").val(),"stayMoney":$(val).find(".stayMoney").val(),
				...
				$("#billTbody").append('<tr class="subData" ><td style="display:none" class="subId"></td><td style="display:none" class="procureSalesId">'+procureSalesId+'</td><td class="documentNum">' + documentNum + '</td><td>' + datetime + '</td><td class="price">' + price + 
					'</td><td class="alreadyPrice clearingMoney">' + clearingMoney + '</td><td><input type="text" class="form-control amountPayment theMoeny" onkeyup="amountPaymentOnKeyUp(this)" attr-name="本次结算"/></td></td>' +
		...
		/*清空单据*/
		function clearBill() {
			$("#billTbody").html('<tr><th>单据编号</th><th>单据日期</th><th>订单金额</th><th>已结算预付金额</th><th>本次结算</th><th>备注</th><th>单据类型</th><th>操作</th></tr>' +
		...
		function getCommodityMsg(value){
			...
			if(value=="-1"){
				...
				$('#bill').parent().html('<select id="bill" ><option value="-1">--请选择单据--</option></select>');
				 ...
				 $.ajax({
		                url: '<%=basePath%>/receivable/bills/subList',
		                type: "POST",
		                dataType: "json",
		                async: false,
		                cache: false,
    	...
    	function getDataJSON(){
    		...
    		//预付款单
    		 var  billsDataJson={"id":$("#edit_id").val(),"customerSupplierId":$("#intercourseUnit").val(),"billsType":4,"bank":$("#bank").val(),
