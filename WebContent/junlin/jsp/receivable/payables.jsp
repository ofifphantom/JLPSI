...
<html lang="en">
			...
			<form class="container">
			 <input type="text" class="form-control hidden" id="edit_id" />
				...
				<div class="row jl-title">
					...
					<i class="r_f">
					<select id="bill" disabled="disabled"  style="width: 250px">
	...
	laydate.render({
		...
		function advancesReceivedAdd() {
			...
			orderBillAdd("应付款单",function(index){
				var orderType=1;
				$("#billTbody .subData").each(function(index, val){
	    			 
		    		  if($(val).find(".ltypeName").html()=="采购退货"){
		    			  orderType=2;
		    		  }
	                ...
	                success: function(data) {
	        			...
	        			//循环加载新数据
	               		for(var i=0;i<subList.length;i++){
	               			...
	               			"<td>是</td><td>"+subList[i].actualMoney+"</td><td>"+subList[i].remark+"</td><td>采购订单</td><td>"+subList[i].rebate+"</td><td>"+subList[i].rebateMoney+"</td></tr>";
			...
			$("#billTbody .subData").each(function(index, val){
 			...
 			if($("#intercourseUnit").val()=="-1"){
				...
				$("#billTbody").append('<tr class="subData"><td style="display:none" class="subId"></td><td style="display:none" class="procureSalesId">'+procureSalesId+'</td><td class="documentNum">' + documentNum + '</td><td>' + datetime + '</td><td class="price">' + price + 
					...
					'<td><input type="text" class="form-control discount" onkeyup="countDiscount(this)" value="100" attr-name="折扣"/></td>' +
					'<td><input type="text" class="form-control discountPrice" disabled="disabled" value="0" /></td>' +
		...
		function getCommodityMsg(value){
			...
			if(value=="-1"){
				...
				$('#bill').parent().html('<select id="bill" ><option value="-1">--请选择单据--</option></select>');
				 ...
				 $.ajax({
		                url: '<%=basePath%>/receivable/bills/returnProcure',
		                type: "POST",
		                dataType: "json",
		                async: false,
		                cache: false,
		                ...
		                success: function(data) {
		               	 ...
		               	 if(data.success){
		               		...
		               		for(var i=0;i<subList.length;i++){
		               			...
		               			var orderTime=getSmpFormatDateByLong(subList[i].orderTime, true);
 		        				$('#bill').append('<option value="'+subList[i].identifier+'" procureSalesId="'+subList[i].procureSalesId+'"attr-date="'+orderTime+'" attr-money="'+toDecimal2(subList[i].orderMoney)+'" attr-alreadyPrice="'+clearingMoney+'" attr-type="1" >'+'采购开单'+subList[i].identifier+'</option>')
		               		...
		               		for(var i=0;i<returnList.length;i++){
 		        				...
 		        				$('#bill').append('<option value="'+returnList[i].identifier+'" procureSalesId="'+returnList[i].procureSalesId+'"attr-date="'+orderTime+'" attr-money="'+toDecimal2(returnList[i].orderMoney)+'" attr-alreadyPrice="'+clearingMoney+'" attr-type="2">'+'采购退货'+returnList[i].identifier+'</option>')
 		        			}
			...
			if($(thisInput).val()!=""){
				...
				/* if(thisSettlement==0){
					...
					if(parentTr.find(".discount").val()!=""){
						...
						parentTr.find(".amountPayment").val(toDecimal2(parentTr.find(".thisSettlement").val()-0));
					}
				}
			}else{
				parentTr.find(".discountPrice").val("");
    	...
    	function getDataJSON(){
    		 var orderType=1;
    		//应付款单
    		 var  billsDataJson={"id":$("#edit_id").val(),"customerSupplierId":$("#intercourseUnit").val(),"billsType":2,"bank":$("#bank").val(),
