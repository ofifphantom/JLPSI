...
<body class="content">
	<div class="page-content clearfix">
		<div id="Member_Ratings">
			<div class="d_Confirm_Order_style" style="margin-top: 10px;">
				<h4 class="text-title">入库单引用</h4>
				<div class="search-div clearfix">
					<div class="clearfix">
						<span class="l_f"> 单号： <input type="text" value=""
							id="query_identifier" />
						</span> <span class="l_f"> 供应商名称： <input type="text" value=""
							id="query_supctoId" />
						...
						<span class="l_f"> 
							状态：<select id="query_state" name="">
								...
								<option value="11">部分入库通知开单</option>	
						...
						<tbody>
							<!-- <tr>
									...
									<td>
										...
										<input type="button" class="btncss edit" onclick="libraryListReference()" value="采购开单" />
									</td>
								</tr> -->
						</tbody>
					</table>
	...
	<div id="libraryListReference" style="display: none; padding: 30px 0;">
		<div class="row">
			<div class="form-group">
				<label for="" class="col-xs-3 control-label">单据类型：</label>
				<div class="col-xs-8">
					<select class="form-control" id="procureOrderType">
						<option value="-1">---请选择采购开单类型---</option>
...
<script>
	...
	jQuery(function($) {
							...
							"aoColumns" : [
									...
									},
									{
										...
										"mRender" : function(data, type, row) {
											...
											+'<input type="button" class="btncss edit" onclick="libraryListReference('+data+')" value="采购开单" /></td>'
		...
		function WarehouseReceiptDetail(id) {
			<%-- $.ajax({
				...
				success : function(data) {
					...
					if(data.isOtherReceipts!=1){
						...
						console.log("isOtherReceipts","else");
			...
			$.ajax({
				type: "post",
				url: "<%=basePath%>purchase/procuretable/inputOrderReferenceDetail",
				dataType : "json",
				data: {
					"id" : id
				},
		...
		/*引用*/
		function libraryListReference(id) {
			publicMessageLayer("采购开单", function() {
				ajaxJquery("GET","<%=basePath%>purchase/procuretable/createProcureBilling",{"id":id,"orderType":1},function(data){
					if(data.success) {
						laysuccess("开单成功，请至采购开单页面查看详情。");
			...
			/* layer.open({
				...
				closeBtn: 1,
				area: ['500px', '210px'],
				content: $("#libraryListReference"),
				btn: ['开单','关闭'],
				yes:function(index){
					if($("#procureOrderType").val()==-1){
						laywarn("请选择单据类型。");
						return;
					}
