							...
							<tbody>
								<!--<tr>
									...
									<td>
										...
										<input type="button" class="btncss edit" onclick="documentPrint()" value="单据打印" />
									</td>
								</tr>-->
							</tbody>
						</table>
							...
							"aoColumns" : [
...

									{
										...
										"mRender" : function(data, type, row) {
											...
											return '<td><input type="button" class="btncss edit" onclick="purchaseOrderDetail('+data+')" value="单据打印" /></td>'
		...
		function purchaseOrderDetail(id) {
			<%--$.ajax({
				...
				success : function(data) {
					...
					$("#lookDiv").append(watermark("打印次数："+data.printNum+"次","#pickingList"));
			...
			$.ajax({
				...
				success: function(res) {
					...
					$.ajax({
						...
						success : function(data) {
							$("#lookDiv").append(watermark("打印次数："+(data.printNum+1)+"次","#pickingList"));
							$('.watermark>div').fontFlex(80,100,10);
						}
					})
				}
			...
			layer.open({
				...
				yes: function(index) {
					MessageLayer("您确定要打印该单据吗？", function() {
						$.ajax({
							url :'<%=basePath%>/purchase/procuretable/updateProcurePrintNum' ,
							type : "POST",
							dataType : "json",
							data: {
								"id" : id
