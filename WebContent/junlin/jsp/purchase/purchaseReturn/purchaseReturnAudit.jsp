...
<head>
						...
						<table class="table table-striped table-hover col-xs-12" id="datatable">
							...
							<tbody>
								<!-- <tr>
									...
									<td>1</td>
									<td>
										<input type="button" class="btncss edit" onclick="purchaseReturnPass()" value="通过" />
										<input type="button" class="btncss edit" onclick="purchaseReturnReject()" value="驳回" />
									</td>
								</tr> -->
							</tbody>
						</table>
	...
	jQuery(function($) {
							...
							"aoColumns" : [
									...
									},
									{
										...
										"mRender" : function(data, type, row) {
											...
											return '<td><input type="button" class="btncss edit" onclick="purchaseReturnPass('+data+')" value="通过" />'
											+'<input type="button" class="btncss edit" onclick="purchaseReturnReject('+data+')" value="驳回" /></td>'
		...
		/*通过*/
		function purchaseReturnPass(id) {
			publicMessageLayer("通过该采购退货单的审批", function() {
				ajaxJquery("POST","<%=basePath%>purchase/procuretable/updateReturnedPurchaseState",{"id" : id,"state":3},function(data){
					if(data.success) {
						laysuccess(data.msg);
		...
		/*驳回*/
		function purchaseReturnReject(id) {
			publicMessageLayer("驳回通过该采购退货单的审批", function() {
				ajaxJquery("POST","<%=basePath%>purchase/procuretable/updateReturnedPurchaseState",{"id" : id,"state":4},function(data){
					if(data.success) {
						laysuccess(data.msg);
						oTable1.fnDraw(false);
					} else {
