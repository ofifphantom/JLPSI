...
<html lang="en">
		...
		<div class="page-content clearfix">
			<div id="Member_Ratings">
				<div class="d_Confirm_Order_style" style="margin-top:10px;">
					<h4 class="text-title">其他发货单开单</h4>
			...
			<form action="" class="container">
				...
				<div class="row">
					<table id="SalesOpeningTable" class="table table-bordered table-striped table-hover col-xs-12" border="" cellspacing="0" cellpadding="0">
						<tr>
							<th>商品</th><th>单价</th><th>税率</th><th>发货数量</th><th>签收数量</th><th>签收金额</th>
						</tr>
								...
								},
								{
									...
									"mRender" : function(data, type, row) {
										if(row.state == 24){
											...
											+JSON.stringify(row)
											+')\' value="详情" /><input type="button" class="btncss edit" onclick=\'salesOpening('
											+ JSON.stringify(row)
											+')\' value="开单" /></td>'
										}else{
											return '<td><input type="button" class="btncss edit" onclick=\'salesOrderDetail('
	...
	function salesOrderDetail(data) {
		...
		var id=data.id;
		var title="其他发货单开单";
		if(data.state == 24) title="其他发货单出库单";
	...
	function salesOpening(row){
		var ids=[];
		ids.push(row.id);
		$("#SalesOpeningTable").html("<tr><th>商品</th><th>单价</th><th>税率</th><th>发货数量</th><th>签收数量</th><th>签收金额</th></tr>");
		ajaxJquery("GET","<%=basePath%>sales/salesOrder/selectOrderDetailById",{"id":row.id},function(data){
			for(let i=0;i<data.salesOrderCommodities.length;i++){
				...
				$("#SalesOpening_goodDiv"+(i+1)+" .SalesOpening_receiving_goods_num").val(data.salesOrderCommodities[i].deliverGoodsNum);
				$("#SalesOpening_goodDiv"+(i+1)+" .SalesOpening_receiving_goods_money").val((data.salesOrderCommodities[i].deliverGoodsMoney).toFixed(2));
