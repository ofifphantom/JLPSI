		...
		<div class="page-content clearfix">
			<div id="Member_Ratings">
				<div class="d_Confirm_Order_style" style="margin-top:10px;">
					<h4 class="text-title">其他发货单领导审核</h4>
	...
	jQuery(function($) {
		...
		function salesOrderTimes() {
			...
			publicMessageLayer("驳回所选订单的审核", function() {
				layer.open({
					...
					btn: ['确认', '取消'],
					yes: function(index) {
						ajaxJquery("GET","<%=basePath%>sales/salesOrder/orderApproveReject",{"ids":ids,"flag":2},function(data){
							if(data.success) {
								laysuccess(data.msg);
		...
		function salesOrderCheck() {
			...
			publicMessageLayer("通过所选订单的审核", function() {
				...
				ajaxJquery("POST","<%=basePath%>sales/salesOrder/updateStateByIds",{"ids":ids,"state":36,"isCheck":1,"reviewerType":1,"msg":"操作成功，已通过！"},function(data){
					if(data.success) {
						laysuccess(data.msg);
						oTable1.fnDraw(false);
						$("#checkAll").removeAttr("checked");
