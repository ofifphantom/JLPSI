			...
			<form class="container">
				...
				<div class="row jl-panel">
					<div class="col-xs-6">
						...
						<div class="form-group">
							<label for="" class="col-xs-4 control-label">折损单审核人:</label>
	...
	jQuery(function($) {
		...
		function salesOrderDetail(id) {
			...
			ajaxJquery("GET","<%=basePath%>sales/salesOrder/selectOrderDetailById",{"id":id},function(data){
				...
				if(data.foldLossIdentifier == null || data.foldLossName == null){
					$(".look_reviewer_id").html("");
				}else{
					$(".look_reviewer_id").html(data.foldLossIdentifier+"("+data.foldLossName+")");
				}
		...
		function salesOrderTimes() {
			...
			publicMessageLayer("驳回所选折损单的审核", function() {
				$.ajax({
					url :'<%=basePath%>sales/salesOrder/updateStateByIdsAndState',
					type: "POST",
					dataType: "json",
					async: false,
					cache: false,
		...
		function salesOrderCheck() {
			...
			publicMessageLayer("通过所选折损单的审核", function() {
				$.ajax({
					url :'<%=basePath%>sales/salesOrder/updateStateByIdsAndState',
					type: "POST",
					dataType: "json",
