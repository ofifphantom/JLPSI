			...
			<div id="Member_Ratings">
				<div class="d_Confirm_Order_style" style="margin-top:10px;">
					...
					<div class="search-div clearfix">
						...
						<span class="l_f"  style="width:417px;"> 
							...
							<span class="l_f" id="select_location0">
							地区：
					...
					<div class="opration-div clearfix">
						...
						<span class="r_f"> 
							<button type="button" class="btncss jl-btn-defult" onclick="customerDel()"><i class="fa fa-trash"></i> 删除</button>
						</span>
						<span class="r_f"> 
			...
			<form class="col-xs-11" id="editDivForm">
				...
				<div class="col-xs-6">
					<div class="form-group" style="display: none;">
						<label for="customer_or_supplier" class="col-xs-4 control-label">客户</label>
						<div class="col-xs-8">
							<input type="text" class="form-control" id="customer_or_supplier"  name="customerOrSupplier" value="1" maxlength="100"/>
						</div>
					</div>
					<div class="form-group">
						<label for="name" class="col-xs-4 control-label"><span class="text-danger">*</span> 客户名称</label>
						<div class="col-xs-8">
							<input type="text"  class="form-control" id="name" name="name" onblur="cky(this)" maxlength="100"/>
						</div>
					</div>
					<div class="form-group">
						<label for="mnemonic_code" class="col-xs-4 control-label"><span class="text-danger">*</span> 客户编码</label>
						<div class="col-xs-8">
							<input type="text" class="form-control" id="mnemonic_code"  name="memoryCode" onblur="cky(this)" maxlength="100"/>
						</div>
					</div>
					<div class="form-group">
						<label for="chooseOne" class="col-xs-4 control-label"><span class="text-danger">*</span> 所属分类</label>
					...
					<div class="form-group">
						<label for="frade" class="col-xs-4 control-label">客户等级</label>
						<div class="col-xs-8">
					...
					<div class="form-group">
						<label for="settlement_type_id" class="col-xs-4 control-label"><span class="text-danger">*</span> 结算方式</label>
						<div class="col-xs-8">
					...
					<div class="form-group">
						<label for="contact_people" class="col-xs-4 control-label"><span class="text-danger">*</span> 联系人</label>
						<div class="col-xs-8">
							<input type="text" class="form-control" id="contact_people" name="contactPeople" onblur="cky(this)" maxlength="100"/>
						</div>
					</div>
					<div class="form-group">
						<label for="phone" class="col-xs-4 control-label"><span class="text-danger">*</span> 联系方式</label>
						<div class="col-xs-8">
					...
					<div class="form-group">
						<label for="delivery_address" class="col-xs-4 control-label"><span class="text-danger">*</span> 送货地址</label>
						<div class="col-xs-8">
				...
				<div class="col-xs-6">
					<div class="form-group">
						<label for="province" class="col-xs-4 control-label"><span class="text-danger">*</span> 地区</label>
					...
					<div class="form-group">
						<label for="ratepaying" class="col-xs-4 control-label"><span class="text-danger">*</span> 纳税识别编号</label>
						<div class="col-xs-8">
						...
						<div class="col-xs-12">
							<div class="form-group">
								...
								<div class="col-xs-10" style="padding-left: 8px;">
									...
									<div class="col-xs-2">
										<input type="text" class="form-control" id="price"  placeholder="请输入产品销售单价" onblur="cky(this)" onkeyup="pressMoney(this)"/>
									</div>
									<div class="col-xs-2" style="padding: 0;">
										<button type="button" class="btncss jl-btn-importent" id="goodsAddBtn" style="margin-top:5px;">新增</button>
										<input type="text" id="goodsObjInput" name="goodsObj" class="hidden" />
									</div>
	...
	* var goodschecked = {
		...
		$(function(){
			...
			$("#goodsAddBtn").click(function() {
					...
					let id = $("#goodsname").val();
					let commodityName = $("#goodsname").find("option:selected").attr("data-commodityName");	
					let specificationName = $("#goodsname").find("option:selected").attr("data-specificationName");	
					let specificationIdentifier = $("#goodsname").find("option:selected").attr("data-specificationIdentifier");
					let baseMiniPrice = $("#goodsname").find("option:selected").attr("data-baseMiniPrice");
					
					let taxes = $("#goodsname").find("option:selected").attr("data-taxes");
						
					let price = $("#price").val();
					
					let $goods=`<tr><td>` + commodityName + `</td>
									<td>` + specificationIdentifier + `</td>
									<td>` + specificationName + `</td>
									<td>` + baseMiniPrice + `</td>
									<td>` + taxes + `</td>
									<td>` + price + `</td>
									<td><input type="button" class="btncss edit" id="" value="删除" onclick="delTr(this,` +id + `)"  /></td>
									</td>
								</tr>`; 
					...
					if(flag) {
						...
						$("#goodsTbody").append($goods);
						$("#goodsObjInput").val(JSON.stringify(goodsObj));
						//console.log($("#goodsObjInput").val());
					} else {
						laywarn("请勿选择重复商品");
					}
					}
	    ...
	    /*删除商品展示列表li*/
	    function delTr(thisIn,id) {
			$(thisIn).parents("tr").remove();
			if($("#goodsTbody tr").length==1){
				$("#goodsTbody").append(`<tr class="tipTr">
						<td colspan="7">请先选择往来产品</td>
						</tr>`)
			}
			$(goodsObj).each(function(i, dom) {
	...
	jQuery(function($) {
		...
		/* 商品下拉框赋值 */
		$.ajax({
			url :'<%=basePath%>/basic/goods/commodity/getAllCommodityByStateAndIsDelete' ,
			type : "POST",
			dataType : "json",
			data: {
			...
			success : function(data) {
				if(data.length==0){
					$("#goodsname").append("<option value='-1' selected>--暂无数据，请到商品配置页面进行添加--</option>");	
				}else{
					for ( var i = 0; i < data.length; i++) {
						var option = $("<option  value='"+data[i].id
								...
								+"' data-baseMiniPrice='"+data[i].baseMiniPrice
								+"' data-baseCommonlyPrice='"+data[i].baseCommonlyPrice
								+"' data-taxes='"+data[i].commodity.taxes+"'>"
								+ data[i].commodity.name 
								+"(规格："+data[i].specificationName
										+",规格编号："+data[i].specificationIdentifier
										+")</option>");
			...
			.dataTable({
				...
				"aoColumns": [{
						...
						"mRender": function(data, type, row) {
							switch (data) {
							case 1:
								return '<td><input type="button" class="btncss edit" onclick=\'customerEdit(' + JSON.stringify(row) + ')\' value="编辑" />'+
								'<input type="button" class="btncss edit" onclick=\'customerDelivery(' + JSON.stringify(row) + ')\' value="送审" /></td>';
								break;
							case 2:
							...
							case 3:
								if(row.useable == 1){
									return '<td><input type="button" class="btncss edit" onclick=\'customerStop(' + JSON.stringify(row) + ')\' value="申请停用" /></td>';
								}else{
									return '<td><input type="button" class="btncss edit" onclick=\'customerToUse(' + JSON.stringify(row) + ')\' value="恢复" /></td>';
								}
	...
	$("#province").change(function(){
		$("province_code").val($("#province").find("option:selected").html());
	})
	$("#city").change(function(){
		$("city_code").val($("#city").find("option:selected").html());
	})
	$("#area").change(function(){
		$("area_code").val($("#area").find("option:selected").html());
	})
		  ...
		  /*送审*/
		  function customerDelivery(rowData){
			  var id = rowData.id;
				var ids=[];
				ids.push(id);
				
				
				publicMessageLayer("送审该客户", function() {
		  ...
		  /**
			*商品下拉框的值改变事件
			*/
			function selectCommodityMsg(e,r){
				let baseCommonlyPrice = $("#goodsname").find("option:selected").attr("data-baseCommonlyPrice");
				
				$("#price").val(baseCommonlyPrice);
			}
		...
		function customerEdit(rowData) {
			...
			$.ajax({
				...
				success : function(data) {
					if(data.length>0) $("#goodsTbody .tipTr").remove();
					for ( var i= 0; i< data.length; i++) {
						let id = data[i].commodityId;
						let identifier = data[i].commoditySpecification.specificationIdentifier;
						let specifications = data[i].commoditySpecification.specificationName;
						let name = data[i].commodity.name;
						let baseMiniPrice = data[i].commoditySpecification.baseMiniPrice;
			...
			if(rowData.frade!=null){
				$("#frade").val(rowData.frade);
			}else{
				$("#frade").val("-1");
			}
			if(rowData.fromType!=null){
				$("#from_type").val(rowData.fromType);
			}else{
				$("#from_type").val("-1");
			}
			...
			if(rowData.invoiceType!=null){
				$("#invoice_type").val(rowData.invoiceType);
			}else{
				$("#invoice_type").val(-1);
			}
			...
			layer.open({
				...
				yes: function(index) {
					 ...
					 if($("#name").val() == "") {
							...
							laywarn("客户编码不能为空!");
							return false;
						}else if($("#classification_one_id").val()==-1||$("#classification_two_id").val()==-1){
							laywarn("所属分类不能为空!");
							return false;
							...
							laywarn("送货地址不能为空!");
							return false;
						}else if(checkSelectLocation() == false){
							laywarn("请选择地区!");
							return false;
					...
					$.ajax({
						url :'<%=basePath%>/basic/supctoManager/checkIsRepeat' ,
						type : "POST",
						dataType : "json",
						data: {
							"id" : rowData.id,
						...
						success : function(data) {
							if(data.success) {
								//console.log("goodsObject : "+$("#goodsObjInput").val());	
					 
								var formData = new FormData($("#editDivForm")[0]);
								//console.log($("#goodsObjInput").val());
								$.ajax({
									url :'<%=basePath%>/basic/supctoManager/updateSupctoById?id='+rowData.id,
									type : "POST",
									dataType : "json",
									async: false,
									cache: false,
		...
		function checkSelectLocation(){
			var res = true;
			$("#select_location select").each(function(){
				if($(this).val() == "-1"&&$(this)[0].style.display != 'none'){
					res = false;
				}
			 })
			 return res;
		...
		function customerAdd() {
			$("#remark").val("无");
			$("#classification_two_id").empty();
			$("#classification_two_id").append('<option value="-1" selected="selected">--请先选择一级分类--</option>');
			$("#customer_or_supplier").val("1");
					...
					$.ajax({
						...
						success : function(data) {
							if(data.success) {
								...
								$.ajax({
									url :'<%=basePath%>/basic/supctoManager/addSupcto',
									type : "POST",
									dataType : "json",
									async: false,
									cache: false,
		...
		/*导出全部*/
		function customerEduce() {
			publicMessageLayer("导出全部下列显示的数据", function() {
						
				var classificationId= encodeURI(encodeURI($("#query_classification_two_id").val())); 	
				var name = encodeURI(encodeURI($("#query_name").val())); 
				var fromType = encodeURI(encodeURI($("#query_from_type").val())); 
				var state = encodeURI(encodeURI($("#query_state").val()));
				var provinceCode = encodeURI(encodeURI(typeof($("#query_province_code").val()) == "undefined"?-1:$("#query_province_code").val()));
				var cityCode = encodeURI(encodeURI((!$("#query_city_code").val() && typeof($("#query_city_code").val())!="undefined" && $("#query_city_code").val()!=0)?-1:$("#query_city_code").val()));
				var areaCode = encodeURI(encodeURI(typeof($("#query_area_code").val()) == "undefined"?-1:$("#query_area_code").val()));
				var operatorIdentifier = encodeURI(encodeURI(""));
				var URL="<%=basePath%>/basic/supctoManager/exportSupcto?flag=1&customerOrSupplier=1&classificationId="+classificationId+"&name="+name+"&fromType="+fromType+"&state="+state+"&provinceCode="+provinceCode+"&cityCode="+cityCode+"&areaCode="+areaCode+"&operatorIdentifier="+operatorIdentifier;	
				    location.href=URL;
					return false;			
			})
		}
		...
		function customerStop(rowData){
			...
			var ids=[];
			ids.push(id);
			publicMessageLayer("申请停用该客户", function() {
				$.ajax({
					url :'<%=basePath%>basic/supctoManager/updateStateByIds',
					type: "POST",
					dataType: "json",
		...
		function customerToUse(rowData){
			publicMessageLayer("恢复使用该客户", function() {
				$.ajax({
