		...
		<div class="page-content clearfix">
			<div id="Member_Ratings">
				<div class="d_Confirm_Order_style" style="margin-top:10px;">
					<h4 class="text-title">仓库维护</h4>
					<div class="search-div clearfix">
						<div class="clearfix">
							<span class="l_f"> 
							仓库编号： <input type="text" value="" id="query_identifier"  onblur="cky(this)"/>
						</span>
							<span class="l_f"> 
							仓库名称： <input type="text" value="" id="query_name"  onblur="cky(this)"/>
						</span>
							<span class="l_f"> 
							操作人姓名： <input type="text"  value="" id="query_operatorIdentifier" onblur="cky(this)"/>
						</span>
						<span class="l_f"> 
							仓库地址： <input type="text"  value="" id="query_area" onblur="cky(this)"/>
						</span>	
						<span class="l_f"> 
		...
		<div id="editDiv" style="display: none;">
			<form class="container">
				...
				<div class="form-group">
					...
					<label for="name" class="col-xs-2 control-label">仓库名称&nbsp;<span class="text-danger">*</span></label>
					<div class="col-xs-4">
						<input type="text" class="form-control" id="name"  onblur="cky(this)"/>
					</div>
				</div>
				<div class="form-group">
					<div class="col-xs-2"></div>
					<label for="position" class="col-xs-2 control-label">仓库地址&nbsp;<span class="text-danger">*</span></label>
					<div class="col-xs-4">
						<input type="text" class="form-control" id="position"  onblur="cky(this)"/>
					</div>
				</div>
	...
	jQuery(function($) {
		oTable1 = $('#datatable').dataTable({
			...
			"ajax": {
				...
				"url": "<%=basePath%>basic/warehouse/dataTables"
			},
			"aoColumns": [{
				"mData": "id",
				"bSortable": false,
		...
		function classifitionEdit(rowData) {
			...
			layer.open({
				...
				yes:function(index){
					...
					$.ajax({
						url :'<%=basePath%>/basic/warehouse/checkIsRepeat' ,
						type : "POST",
						dataType : "json",
						data: {
							"id" : rowData.id,
						...
						success : function(data) {
							if(data.success) {
								$.ajax({
									url :'<%=basePath%>/basic/warehouse/updateWarehouseById' ,
									type : "POST",
									dataType : "json",
									data: {
										"id" : rowData.id,
				...
				yes:function(index){
					...
					$.ajax({
						...
						success : function(data) {
							if(data.success) {
								$.ajax({
									url :'<%=basePath%>/basic/warehouse/addWarehouse' ,
									type : "POST",
									dataType : "json",
									data: {
										"name" : $("#name").val(),
		...
		function classifitionDel() {
			...
			publicMessageLayer("删除选中数据", function() {
				$.ajax({
					url :'<%=basePath%>basic/warehouse/deleteWarehouseByIds',
					type: "POST",
					dataType: "json",
					async: false,
					cache: false,
		...
		function classifitionEduce(){
			publicMessageLayer("导出全部下列显示的数据", function() {
				...
				var URL="<%=basePath%>basic/warehouse/exportWarehouse?identifier="+identifier+"&name="+name+"&operatorIdentifier="+operatorIdentifier;	
				    location.href=URL;
					return false;			
			})
		}
