		...
		oTable1 = $('#datatable')
		  ...
		  function customerDelivery(rowData){
			  var id = rowData.id;
				...
				publicMessageLayer("送审该供应商", function() {
	...
	function customerDetail(rowData) {
		...
		function customerEduce() {
			publicMessageLayer("导出全部下列显示的数据", function() {
				...
				var provinceCode = encodeURI(encodeURI(""));
				var cityCode = encodeURI(encodeURI(""));
				var areaCode = encodeURI(encodeURI(""));
				var operatorIdentifier = encodeURI(encodeURI(""));
				var URL="<%=basePath%>/basic/supctoManager/exportSupcto?flag=1&customerOrSupplier=2&classificationId="+classificationId+"&name="+name+"&fromType="+fromType+"&state="+state+"&provinceCode="+provinceCode+"&cityCode="+cityCode+"&areaCode="+areaCode+"&operatorIdentifier="+operatorIdentifier;	
				    location.href=URL;
					return false;			
			})
		}
