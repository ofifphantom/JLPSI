		...
		<div class="page-content clearfix">
			<div id="Member_Ratings">
				<div class="d_Confirm_Order_style" style="margin-top:10px;">
					<h4 class="text-title">供应商二级分类</h4>
					<div class="search-div clearfix">
						<div class="clearfix">
							<span class="l_f"> 
							分类编号： <input type="text" value="" id="searchIdentifier" />
						</span>
							<span class="l_f"> 
							分类名称： <input type="text" value="" id="searchName"/>
						</span>
						<span class="l_f"> 
							一级分类： <select id="searchChooseOne"></select>
						</span>
							<span class="l_f"> 
							操作人姓名： <input type="text"  value="" id="searchOperatorName"/>
						</span>
						<span class="l_f"> 
		...
		<div id="editDiv" style="display: none;">
			<form class="container">
				<div class="form-group">
					...
					<label for="chooseOne" class="col-xs-2 control-label">所属分类&nbsp;<span class="text-danger">*</span></label>
					<div class="col-xs-4">
						<select id="chooseOne" class="form-control">
		...
		oTable1 = $('#datatable')
			.dataTable({
				...
				"ajax": {
					...
					"data": function(d) {
						return $.extend({}, d, {
							...
							"name": $("#searchName").val(),
							"oneClassification": $("#searchChooseOne").val(),
							"operatorName": $("#searchOperatorName").val(),
							"queryTime":$("#query_time").val()		
						});
					},
					"url": "<%=basePath%>basic/classification/getClassificationMsg?type=2&oneOrTwo=2"
				},
				...
				"aoColumns": [{
						...
						"mRender": function(data,type,row) {
							if(data!=null&&data!=""){
								if(row.pClassification!=null){
									return row.pClassification.name;
								}
								else{
									return "";
								}
	...
	function getDataToOneClassification(){
		$.ajax({
			...
			success: function(data) {
				$("#searchChooseOne").html("");
				$("#chooseOne").html("");
				if(data.length==0){
					$("#chooseOne").append("<option value='-1' selected>--暂无数据，请到一级分类页面进行添加。--</option>");	
					$("#searchChooseOne").append("<option value='-1' selected>--暂无数据，请到一级分类页面进行添加。--</option>");	
				}
				else{
					$("#chooseOne").append("<option value='-1' selected>--请选择--</option>");
					$("#searchChooseOne").append("<option value='-1' selected>--请选择--</option>");
				...
				yes: function(index) {
					...
					$.ajax({
						...
						success: function(data) {
							if(data.success) {
								$.ajax({
									url: '<%=basePath%>basic/classification/addTwoClassificationMsg',
									type: "POST",
									dataType: "json",
									async: false,
									cache: false,
		...
		function classifitionDel() {
			...
			$.ajax({
				url: '<%=basePath%>basic/classification/decideTwoClassificationIsExits?page=2',
				type: "POST",
				dataType: "json",
				async: false,
				cache: false,
		...
		function classifitionEduce() {
				...
				var export_oneOrTwo=encodeURI(encodeURI(2));
				...
				var export_type=encodeURI(encodeURI(2));
