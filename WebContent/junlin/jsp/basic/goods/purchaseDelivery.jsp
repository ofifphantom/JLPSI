	...
	function getSupctoMsgByCustomerOrSupplier(){
		...
		function goodsTimes(){
			...
			publicMessageLayer("驳回下列所选商品的审核", function() {
				if(ids1.length>0 && ids2.length>0){
					//console.log("111111");
					ajaxJquery("POST","<%=basePath%>basic/goods/commodity/operateCommodityState",{"ids":ids1,"operate":4},function(data){
						if(data.success) {
							ajaxJquery("POST","<%=basePath%>basic/goods/commodity/operateCommodityState",{"ids":ids2,"operate":11},function(data){
								if(data.success) {
									laysuccess(data.msg);
		...
		function goodsCheck() {
			...
			publicMessageLayer("通过下列所选商品的审核", function() {
				if(ids1.length>0 && ids2.length>0){
					//console.log("111111");
					ajaxJquery("POST","<%=basePath%>basic/goods/commodity/operateCommodityState",{"ids":ids1,"operate":5},function(data){
						if(data.success) {
							ajaxJquery("POST","<%=basePath%>basic/goods/commodity/operateCommodityState",{"ids":ids2,"operate":-1},function(data){
								if(data.success) {
									laysuccess(data.msg);
									oTable1.fnDraw(false);
									$("#checkAll").removeAttr("checked");
					...
					//console.log("11111133333");
					ajaxJquery("POST","<%=basePath%>basic/goods/commodity/operateCommodityState",{"ids":ids2,"operate":14},function(data){
						if(data.success) {
							laysuccess(data.msg);
							oTable1.fnDraw(false);
							$("#checkAll").removeAttr("checked");
