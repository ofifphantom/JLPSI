...
<body class="content" id="goodsManagerPage">
			...
			<div class="d_Confirm_Order_style" style="margin-top: 10px;">
				...
				<div class="search-div clearfix">
					<div class="clearfix">
						...
						<option value="3">销售审核通过</option>
						<option value="1">已停用</option>
						<option value="4">已删除</option>
						<option value="2">其他</option>
						</select>
						</span> 
						<span class="l_f" style="padding-left:38px"> 分类：
	...
	<div id="editDiv" style="display: none;">
		<form class="container">
			...
			<div class="row jl-panel">
				<div class="col-xs-6">
					...
					<div class="form-group">
						<label for="name" class="col-xs-4 control-label">产品名称</label>
						<div class="col-xs-8">
							<input type="text" class="form-control" id="edit_commodity_name" name="goodsname" data-provide="typeahead" placeholder="输入名称可自动进行检索"  onkeyup="searchCommodityMsgByName(this)"  maxlength="100"/>
						</div>
					</div>
					<div class="form-group">
						<label for="classification_id_one" class="col-xs-4 control-label">货品类别</label>
			...
			<div class="text-danger">注：该页面所有字段（助记码、基本信息、简称、条形码除外）均为必填</div>
		</form>
...
<script>
    	...
    	$('#edit_commodity_name').typeahead({
    		...
    		updater: function (obj) {
	            var str=obj.split(" ");
	            searchCommodityMsgByNameCheck(str[1]);
	            return str[1];
	        }
    	})
			...
			oTable1 = $('#datatable')
				.dataTable({
					...
					"aoColumns": [{
							...
							"sClass": "center",
							"mRender": function(data,type,row) {
								 if(row.isDelete==1&&row.state!=10){
									return "已停用";
								}
								else{
									switch(data){
								...
								if(row.isDelete==1&&row.state!=10){
									...
									if(row.state==1||row.state==4){
										...
										if(row.tempState == 10 || row.tempState == 12||row.tempState == 14){
											...
											return '<td><input type="button" class="btncss edit" onclick="goodsStop('+data+',1)" value="停用" />'
											+'<input type="button" class="btncss edit"' +
											'onclick="goodsEdit(' + data + ',2)" value="修改" /></td>'		
										}
																
									}
									else if(row.state==10){
										return '<td><input type="button" class="btncss edit" value="已删除" disabled="disabled" /></td>'
									}
		...
		function searchCommodityMsgByName(e) {
			...
			if(typeof($(e).val())!="undefined"&&$(e).val()!=""){
				...
				$.ajax({
					...
					success: function(data) {
						...
						if(data.length>0) {
							$("#classification_id_one").val(data[0].classification.parentId);
							$("#edit_classification_id").empty();
							var option = $("<option selected value='" + data[0].classification.id + "'>" +
								data[0].classification.name + "</option>");
							$("#edit_classification_id").append(option);
							$("#classification_id_one").attr("disabled", "true");
							$("#edit_classification_id").attr("disabled", "true");
							$("#commodity_id").val(data[0].id);
							$("#commodity_identifier").val(data[0].identifier);
							$("#edit_basics_information").val(data[0].basicsInformation);
							$("#edit_attribute").val(data[0].attribute);
							$("#edit_shout_name").val(data[0].shoutName);
							$("#edit_supcto").val(data[0].supctoId);
							$("#edit_mnemonic_code").val(data[0].mnemonicCode);
							$("#edit_brand").val(data[0].brand);
							$("#edit_zero_stock").val(data[0].zeroStock);
							$("#edit_taxes").val(data[0].taxes);
							$("#is_assemble").val(data[0].isAssemble);
							$("#is_presell").val(data[0].isPresell);
		...
		/* 填写商品名称后 搜索该名称对应的供货商及分类信息，显示在页面中 */
		function searchCommodityMsgByNameCheck(str) {
		...
		function unitAdd(thisbtn,addOrUpdate){
			...
			let str=`<div class="unit clearfix unitInfo" id="unit`+unit_index_num+`" attr-id="`+thispanelid+`">
				<span class="close_unit" onclick="unitDel(this)"><i class="fa fa-times"></i></span>
							...
							<input type="text" class="form-control unit_id hidden"/>
								<div class="form-group">
									...
									<div class="col-xs-8">
										<input type="text" class="form-control unit-name" id="name`+unit_index_num+`" onblur="decideUnitNameIsRepeat(this)" value="箱" disabled="disabled"/>
									</div>
								</div>
								<div class="form-group">
								...
								<div class="form-group hidden">
									...
									<div class="col-xs-8">
										<input type="text" class="form-control commonly_price" value="1" id="commonly_price " disabled="disabled" placeholder="由销售填写"/>
									</div>
								</div>
								<div class="form-group hidden">
									<label class="col-xs-4 control-label ">最低销售价格</label>
									<div class="col-xs-8">
										<input type="text" class="form-control mini_price" value="1" id="mini_price " onkeyup="pressMoney(this)"/>
									</div>
								</div>
							</div>
							<div class="col-xs-6 ">
								...
								<div class="form-group hidden">
									...
									<div class="col-xs-8">
										<input type="text" class="form-control sales_unit" value="1" id="sales_unit " onblur="cky(this)" onkeyup="pressInteger(this)" maxlength="9" value="袋" disabled="disabled" />
									</div>
								</div>
								<div class="form-group hidden">
									<label class="col-xs-4 control-label">采购单位</label>
									<div class="col-xs-8"> 
										<input type="text" class="form-control purchasing_unit" value="1" id="purchasing_unit " onblur="cky(this)" onkeyup="pressInteger(this)" maxlength="9" value="袋、箱" disabled="disabled"/>
									</div>
								</div>
								<div class="form-group hidden">
									<label class="col-xs-4 control-label">仓库单位</label>
									<div class="col-xs-8">
										<input type="text" class="form-control warehouse_unit" value="1" id="warehouse_unit " onblur="cky(this)" onkeyup="pressInteger(this)" maxlength="9" value="袋、箱" disabled="disabled"/>
									</div>
								</div>
							</div>
						</div>`;
						if(addOrUpdate==1){
							...
							$("#goodstypeDiv"+thispanelid+" .unitDiv input[type='text']").each(function(){
							    ...
							    if(!$(this).hasClass("hidden")&&!$(this).hasClass("commonly_price")&&!$(this).hasClass("bar_code")){		
							    	 if($(this).val()==""){
									    	hasEmpty=true;
						...
						//判断单位是否有两个，如果有两个就将其隐藏
						if($(thisbtn).parents(".goodsSpecificationDiv").find(".unitInfo").length==2){
		...
		function unitAddOrUpdateFirst(num){
			...
			let str=`<div class="unit clearfix unitInfo" id="unit`+num+`" attr-id="`+num+`">
				<div class="row">
					<div class="col-xs-6">
						<div class="form-group">
							<input type="text" class="form-control unit_id hidden"/>
					...
					<div class="form-group">
						...
						<div class="col-xs-8">
							<input type="text" class="form-control unit-name" id="name1"
								onblur="decideUnitNameIsRepeat(this)" value="袋" />
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-xs-8">
							<input type="text" class="form-control mini_price"
								id="mini_price " onkeyup="pressMoney(this)" />
						</div>
					</div>
				</div>
		...
		function unitDel(thisspan){
			...
			function checkunit(){
				//判断单位是只有一个，如果有1个就将其显示
				if($(thisspan).parents(".goodsSpecificationDiv").find(".unitInfo").length==2){
					$(thisspan).parents(".unitDivAll").find(".btnUnitAdd").removeClass("hidden");
				}
			}
		}
					...
					<div class="unit clearfix " id="inventory`+inventory_index_num+`" attr-id="`+panelid+`">
						...
						<div class="col-xs-6 ">
							...
							<div class="form-group">
								...
								<div class="col-xs-8">
									<input type="text" class="form-control commodity_num" id="commodity_num" onblur="cky(this)" onkeyup="pressInteger(this)" maxlength="9" value="0" disabled="disabled"/>
								</div>
							</div>
						</div>
				...
				success: function(data) {
					...
					case 3:
						...
						case 13:
							...
							break;
						case 14:
							$("#look_state").html("采购修改总经理审核中");
							break;
						case 15:
							$("#look_state").html("采购修改总经理审核驳回");
							break;
						default:
					...
					//修改审批中的详情展示处理
					 if(data.tempState == 10||data.tempState == 14){
						 $("#look_miniOrderQuantity").html(data.tempMiniOrderQuantity);
						 $("#look_addOrderQuantity").html(data.tempAddOrderQuantity);
						 $("#look_warningNumber").html(data.tempWarningNumber);
						 $("#look_taxes").html(data.commodity.tempTaxes);
						 $("#look_inventory1 .mini_inventory").html(data.tempMiniInventory);
						 $("#look_inventory1 .max_inventory").html(data.tempMaxInventory);
						 if(data.inventories.length==0 && data.tempMiniInventory!=null){//新增了存货信息
							 ...
							 $("#look_inventory_div").html("");
									...
									$("#look_inventory1 .warehouse_id").html(data.specWarehouseName);
									$("#look_inventory1 .mini_inventory").html(data.tempMiniInventory);
									$("#look_inventory1 .max_inventory").html(data.tempMaxInventory);
									//账面金额：账面库存*成本单价
									$("#look_inventory1 .carrying_amount").html(0);
									
									$("#look_inventory1 .occupied_inventory").html(0);
									//实际数量：账面库存+占用数量
									$("#look_inventory1 .inventory").html(0);
									$("#look_inventory1 .cost_price").html(0);
		...
		function goodsEdit(id,type) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					//单位信息
					commonlyPrice = data.units[0].commonlyPrice;
			...
			if(type==2){
				//停用的编辑
				$(".edit_quality_period_unit").attr("disabled", "true");
				$(".edit_quality_period").attr("disabled", "true");
				$(".mini_price").attr("disabled", "true");
				$(".warehouse_id").attr("disabled", "true");
				$(".btnUnitAdd").addClass("hidden");
				$(".close_unit").addClass("hidden");
				$("#unit0 .commonly_price").val(commonlyPrice);
		...
		/*新增*/
		function goodsAdd() {
			$("#addSpecificationBtn").removeAttr("hidden");
			$("#edit_zero_stock").val(0);
			$("#is_assemble").val(2);
			$("#is_presell").val(2);
			editOrAdd=1;
			getDataToOneClassification(2);
		...
		function decideInputAndSelectHasValue(){
			...
			$("#editDiv input").each(function(index, val) {
				if($(val).val() == "" && res &&!$(val).hasClass("hidden")&&!$(val).hasClass("commonly_price")&&!$(val).hasClass("bar_code")&&$(val).attr("id")!="edit_shout_name"&&$(val).attr("id")!="edit_mnemonic_code"&&$(val).attr("id")!="edit_basics_information") {
		...
		function commodityDataToJSON(){
			...
			$("#goodstypeDiv .goodsSpecificationDiv").each(function(index, goodsval){
				...
				$(goodsval).find(".unitDiv .unit").each(function(index, val){
					...
					var unitDataJSON={"basicUnit":isBaseUnitChecked/* ,"miniPurchasing":isminiPurchasingChecked */,"barCode":$(val).find(".bar_code").val(),"salesUnit":"箱","purchasingUnit":"袋、箱",
							"warehouseUnit":"袋、箱","name":$(val).find(".unit-name").val(),"ratioDenominator":denominator,"ratioMolecular":molecular,"miniPrice":$(val).find(".mini_price").val()};	
				...
				console.log("commodityDataJSON",commodityDataJSON);
			...
			let goodstypestr=`<div class="row jl-panel goodsSpecificationDiv" id="goodstypeDiv`+goodstypeDivid+`">
				<input type="text" class="form-control edit_specification_id hidden"  />
					...
					<div class="unitDivAll">
						<div class="unit-title row"><span>单位信息</span><button type="button" class="btncss jl-btn-defult btnUnitAdd" onclick="unitAdd(this,1)" attr-id="`+goodstypeDivid+`">新增单位信息</button></div>
		...
		function goodstypeAddfirst(){
			...
			let goodstypestr=`
				<div class="row jl-panel goodsSpecificationDiv" id="goodstypeDiv0">
					...
					<div class="unitDivAll">
						<div class="unit-title row"><span>单位信息</span><button type="button" class="btncss jl-btn-defult btnUnitAdd" onclick="unitAdd(this,1)" attr-id="0">新增单位信息</button></div>
