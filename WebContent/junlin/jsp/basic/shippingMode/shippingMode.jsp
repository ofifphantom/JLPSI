			...
			<div id="Member_Ratings">
				<div class="d_Confirm_Order_style" style="margin-top:10px;">
					...
					<div class="opration-div clearfix">
						<!-- <span class="r_f"> 
							<button type="button" class="btncss jl-btn-defult" onclick="shippingModeEduce()" style="margin-right: 0;"><i class="fa fa-download"></i> 导出全部</button>
							
						</span> -->
						<span class="r_f"> 
							<button type="button" class="btncss jl-btn-defult" onclick="shippingModeDel()"><i class="fa fa-trash"></i> 删除</button>
						</span>
						<span class="r_f"> 
							<button type="button" class="btncss jl-btn-defult" onclick="shippingModeAdd()"><i class="fa fa-plus"></i> 新增</button>
						</span>
						<span class="jl_f_l">
	...
	jQuery(function($) {
		oTable1 = $('#datatable').dataTable({
			...
			"ajax": {
				...
				"url": "<%=basePath%>basic/shippingMode/dataTables"
			},
			"aoColumns": [{
				"mData": "id",
				"bSortable": false,
				...
				"mRender": function(data,type,row) {
					if(row.isDelete == 1){
						return '<td><input type="button" class="btncss edit" value="已删除" disabled/>'
				     	...
				     	return '<td><input type="button" class="btncss edit" onclick=\'shippingModeEdit(' + JSON.stringify(row) + ')\' value="编辑" />';
					}
				}
			}],
		...
		/*修改*/
		function shippingModeEdit(rowData) {
			...
			layer.open({
				...
				yes:function(index){
					...
					$.ajax({
						url :'<%=basePath%>/basic/shippingMode/selectShippingModeNamePreventRepeatedit' ,
						type : "POST",
						dataType : "json",
						data: {
							"id" : rowData.id,
						...
						success : function(data) {
							if(data.success) {
								$.ajax({
									url :'<%=basePath%>/basic/shippingMode/updateShippingModeById' ,
									type : "POST",
									dataType : "json",
									data: {
										"id" : rowData.id,
		...
		function shippingModeAdd() {
			layer.open({
				...
				yes:function(index){
					...
					$.ajax({
						url :'<%=basePath%>/basic/shippingMode/selectShippingModeNamePreventRepeatAdd' ,
						type : "POST",
						dataType : "json",
						data: {
							"name" : $("#name").val()
						...
						success : function(data) {
							if(data.success) {
								$.ajax({
									url :'<%=basePath%>/basic/shippingMode/addShippingMode' ,
									type : "POST",
									dataType : "json",
									data: {
										"name" : $("#name").val()
		...
		function shippingModeDel() {
			...
			$.ajax({
				url :'<%=basePath%>basic/shippingMode/afterDelDecide',
				type: "POST",
				dataType: "json",
				async: false,
				cache: false,
				...
				success: function(data) {
					if(data.success) {
						publicMessageLayer("删除选中数据", function() {
							$.ajax({
								url :'<%=basePath%>basic/shippingMode/deleteShippingModeByIds',
								type: "POST",
								dataType: "json",
								async: false,
								cache: false,
		...
		function shippingModeEduce(){
			publicMessageLayer("导出全部下列显示的数据", function() {
				...
				var URL="<%=basePath%>basic/shippingMode/exportShippingMode?name="+name+"&operatorIdentifier="+operatorIdentifier;	
				    location.href=URL;
					return false;			
			})
		}
