		...
		<div class="page-content clearfix">
			<div id="Member_Ratings">
				<div class="d_Confirm_Order_style" style="margin-top:10px;">
					<h4 class="text-title">部门维护</h4>
					<div class="search-div clearfix">
						<div class="clearfix">
							<span class="l_f"> 
							部门编号： <input type="text" value="" id="query_identifier"  onblur="cky(this)"/>
						</span>
							<span class="l_f"> 
							部门名称： <input type="text" value="" id="query_name"  onblur="cky(this)"/>
						</span>
							<span class="l_f"> 
	...
	jQuery(function($) {
		oTable1 = $('#datatable').dataTable({
			...
			"ajax": {
				...
				"url": "<%=basePath%>basic/department/dataTables"
			},
			"aoColumns": [{
				"mData": "id",
				"bSortable": false,
		...
		function classifitionEdit(rowData) {
			...
			layer.open({
				...
				yes:function(index){
					...
					$.ajax({
						url :'<%=basePath%>/basic/department/checkIsRepeat' ,
						type : "POST",
						dataType : "json",
						data: {
							"id" : rowData.id,
						...
						success : function(data) {
							if(data.success) {
								$.ajax({
									url :'<%=basePath%>/basic/department/updateDepartmentById' ,
									type : "POST",
									dataType : "json",
									data: {
										"id" : rowData.id,
				...
				yes:function(index){
					...
					$.ajax({
						...
						success : function(data) {
							if(data.success) {
								$.ajax({
									url :'<%=basePath%>/basic/department/addDepartment' ,
									type : "POST",
									dataType : "json",
									data: {
										"name" : $("#name").val()
		...
		function classifitionDel() {
			...
			publicMessageLayer("删除选中数据", function() {
				$.ajax({
					url :'<%=basePath%>basic/department/deleteDepartmentByIds',
					type: "POST",
					dataType: "json",
					async: false,
					cache: false,
		...
		function classifitionEduce(){
			publicMessageLayer("导出全部下列显示的数据", function() {
				...
				var URL="<%=basePath%>basic/department/exportDepartment?identifier="+identifier+"&name="+name+"&operatorIdentifier="+operatorIdentifier;	
				    location.href=URL;
					return false;			
			})
		}
